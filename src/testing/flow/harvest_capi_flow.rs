...
mod test {
    ...
    pub async fn harvest_capi_precs(
        ...
        let central_funds_decimal = send_to_escrow_after_investor_locked.0.as_decimal()
            / capi_deps.escrow_percentage.value();
        // unwrap: we ensured with parameters above that central_funds_decimal is an integer
        let central_funds = FundsAmount(central_funds_decimal.to_u64().unwrap());
