...
pub fn withdrawal_tx_note_prefix_with_project_id_base64(project_id: u64) -> String {
    let str = withdrawal_tx_note_prefix(project_id);
...
pub fn generate_withdrawal_tx_note(project_id: u64, body: &str) -> Vec<u8> {
    format!("{}{}", withdrawal_tx_note_prefix(project_id), body)
        .as_bytes()
        .to_vec()
}

/// Extract the note body
pub fn strip_prefix_from_note(note: &[u8], project_id: u64) -> Result<String> {
    let note_decoded_bytes = &BASE64.decode(note)?;
    let note_str = std::str::from_utf8(&note_decoded_bytes)?;

    Ok(note_str
        .strip_prefix(&withdrawal_tx_note_prefix(project_id))
        .ok_or_else(|| {
...
fn withdrawal_tx_note_prefix(project_id: u64) -> String {
    format!("{}{}", capi_note_prefix(), project_id)
}
