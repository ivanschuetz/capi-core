...
pub async fn submit_create_assets(
    ...
    let shares_asset_id = wait_for_pending_transaction(algod, &create_shares_tx_res.tx_id)
        .await?
        .ok_or(anyhow!("No pending tx to retrieve shares asset id"))?
        .asset_index
        .ok_or(anyhow!("Shares asset id in pending tx not set"))?;

    let votes_asset_id = wait_for_pending_transaction(algod, &create_votes_tx_res.tx_id)
        .await?
        .ok_or(anyhow!("No pending tx to retrieve votes asset id"))?
        .asset_index
        .ok_or(anyhow!("Votes asset id in pending tx not set"))?;
